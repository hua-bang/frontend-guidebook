...
源码通过`parse`生成了`AST`之后，需要对`AST`进行遍历和修改。那么`transform`的流程是怎么样的呢。

`babel`会遍历`AST`，遍历过程中处理到不同的`AST`节点会调用不同的`visitor`来实现`transform`。这是访问者模式的应用。

## 路径和作用域
`babel`的`AST`本身更多的就是描述数据的一个抽象，包含源码的一些信息，如果我们要操作`AST`时候需要拿到其他节点信息，或者是你做增删改的方法，这些其实都是要用到`path`。

![image-20220323111037666](../../../assets/image-20220323111037666.png)

### path 的属性

由于`@babel/traverse`中并没有详细的问题进行记录，所以在这里可以参考以下的链接。
- [Path](https://github.com/jamiebuilds/babel-handbook/blob/master/translations/zh-Hans/plugin-handbook.md#paths%E8%B7%AF%E5%BE%84)
- [Babel-traverse-api-doc](https://evilrecluse.top/Babel-traverse-api-doc/#/?id=nodepath-%e5%b1%9e%e6%80%a7)
...
作用域中保存的是声明的变量和对应的值，每个声明为一个`binding`（绑定）。

```js
const a = 1;
```

实际的`path.scope.bindings`是这样的
