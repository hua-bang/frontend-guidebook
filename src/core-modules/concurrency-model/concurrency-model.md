...
在了解JavaScript单线程、非阻塞机制之前，先了解机组易混淆的概念。

进程和线程的概念与关系：

- **进程(progress)**:进程是系统资源分配和调度的单元。一个运行着的程序对应了一个进程。一个进程包括了运行中的程序和程序使用到的内存和系统资源。
- **线程(Thread)**: 线程是进程下的执行者， 一个进程至少开启一个线程,也可以开启多个线程。
...
Javascript是一门**单线程**语言，与它的用途有关。作为浏览器脚本语言，JavaScript 的主要用途是与用户互动，以及操作 DOM。这决定了它只能是单线程，否则会带来很复杂的同步问题。比如，假定 JavaScript 同时有两个线程，一个线程在某个 DOM 节点上添加内容，另一个线程删除了这个节点，这时浏览器应该以哪个线程为准呢？
...
Javascript的程序任务可以分为两种

- **同步任务**：同步任务在主线程上调用之后需要一直等待，只有当前任务执行完之后，才能执行下一个任务。
- **异步任务**：异步任务会在主线程先执行一部分，然后退出主线程至专用线程中执行。在异步任务准备就绪后，会被推入任务队列等待(Task Queue),当主线程空闲时，JavaScript 解释器会执行一次事件循环（EventLoop）将事件队列中首个事件推进主线程执行。
...
![异步任务执行机制图解](https://tsejx.github.io/javascript-guidebook/static/workflow.525aebdc.png)